...
def format_tron_action_dict(action_config: TronActionConfig, use_k8s: bool = False):
    ...
    if executor in KUBERNETES_EXECUTOR_NAMES and use_k8s:
        ...
        if executor != "spark":
            # TODO(PAASTA-17545,TRON-1714): we'll want a better feature toggle for this depending
            # on when we're able to start rolling things out
...
def validate_complete_config(
    ...
    proc = subprocess.run(
        [
            "/nail/home/luisp/pg/Yelp/Tron/.tox/py36/bin/tronfig",
            "-",
            "-V",
            "-n",
            service,
